...
$sql0="SELECT idP FROM pedidos2 WHERE codigoP = '".$pedidoId. "'";

$result0=mysqli_query($conexion,$sql0);

while($mostrar0=mysqli_fetch_array($result0)){
                    $pedidoId=$mostrar0['idP'];    
                   }
                   


$query=mysqli_query($conexion,"SELECT listaEmpaque.*, pedidos2.`codigoP` AS pedido FROM listaEmpaque INNER JOIN pedidos2 ON listaEmpaque.`pedidoId` = pedidos2.`idP` WHERE  shade LIKE '%$color%' AND mold LIKE '%$referencia%' AND lote LIKE '%$lote%' AND pedidoId LIKE '%$pedidoId%' AND Fecha BETWEEN '$desde' AND '$hasta' ");
