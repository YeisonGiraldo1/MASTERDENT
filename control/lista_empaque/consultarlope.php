...
$query=mysqli_query($conexion,"SELECT listaEmpaque.*, pedidos2.`codigoP` AS pedido FROM listaEmpaque INNER JOIN pedidos2 ON listaEmpaque.`pedidoId` = pedidos2.`idP`  WHERE lote LIKE '%$lote%'   AND pedidoId LIKE '%$pedidoId%'  AND Fecha LIKE '%$fecha%' ");
