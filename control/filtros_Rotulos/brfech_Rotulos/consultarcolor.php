...
$query=mysqli_query($conexion,"SELECT rotulos2.*,rotulos2.id AS rotid, colores2.*,colores2.nombre AS color, estaciones2.*,estaciones2.nombre AS verestacion , lotes2.*,lotes2.nombreL AS verlote,referencias2.*, pedidos2.*,referencias2.nombre AS vereferencia, pedidos2.codigoP AS verpedido FROM rotulos2 INNER JOIN colores2 ON rotulos2.`colorId` = colores2.`id` INNER JOIN estaciones2 ON rotulos2.`estacionId2`=estaciones2.`id`INNER JOIN lotes2 ON rotulos2.`loteId`=lotes2.`id` INNER JOIN referencias2 ON rotulos2.`referenciaId`=referencias2.`id` INNER JOIN pedidos2 ON rotulos2.`pedido`=pedidos2.`idP`WHERE colores2.nombre LIKE '$color'AND Fecha BETWEEN '$desde' AND '$hasta'");
...
<th  hidden><?php   echo $data['id'];?></th>
    ...
    <th><?php   echo $data['fecha'];?></th>
      ...
      <td><?php   echo $data['total'];?></td>
        ...
        <td><div style="display:flex;"><a  href="editar4.php?id=<?php    echo $data['rotid'];?>&desde=<?php  echo $desde; ?>&hasta=<?php  echo $hasta;?>">EDITAR</a>&nbsp;&nbsp;&nbsp;
           <a  href="eliminar4.php?id=<?php  echo $data['rotid'];?>&&color=<?php    echo $data['color'];?>&&referencia=<?php    echo $data['referenciaId'];?>&&lote=<?php    echo $data['loteId'];?>&&pedido=<?php    echo $data['pedido'];?>&&desde=<?php  echo $desde;?>&&hasta=<?php echo $hasta  ?>">ELIMINAR</a>
      </div> </td>
      <?php  }?>
    </tr>
