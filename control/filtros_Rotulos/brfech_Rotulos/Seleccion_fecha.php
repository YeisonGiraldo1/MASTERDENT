...
// consulto todos los rótulos de esta fecha y los almaceno en un arreglo 
   ...
   if ($id != ''){
    ...
    $consultaSuma = "select sum(total) as totales FROM rotulos2 INNER JOIN referencias2 ON rotulos2.`referenciaId`= referencias2.`id` INNER JOIN colores2 ON rotulos2.`colorId` = colores2.`id` INNER JOIN pedidos2 ON rotulos2.`pedido` = pedidos2.`idP` INNER JOIN lotes2 ON rotulos2.`loteId`= lotes2.`id` WHERE Fecha BETWEEN '$desde' AND '$hasta' AND " ;
    ...
    $sqlSeparados= "SELECT separado, rotuloId, rotulos2.fecha as fecha FROM pedidoDetalles INNER JOIN rotulos2 ON pedidoDetalles.rotuloId = rotulos2.id WHERE Fecha BETWEEN '$desde' AND '$hasta'  AND separado is not null";
...
<br>
<center>
<div><h1>Producción del <?php echo $desde;?>&nbsp;&nbsp;al&nbsp;&nbsp;<?php  echo $hasta  ?></h1></div>
<br><br>
                ...
                <label for="linea" class="form-label">Línea</label>
                 <select class="form-select" id="linea" name="linea" aria-label="Default select example">
                   ...
                   <br></br>
                    ...
                    <input name="desde" type="hidden" value=" <?php
                        echo $desde;  
                    ?>">
                    <input name="hasta" type="hidden" value=" <?php
                        echo $hasta;  
                    ?>">
